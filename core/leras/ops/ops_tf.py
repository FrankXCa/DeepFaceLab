# DEAD TensorFlow code - preserved for reference only (Phase 3C).
#
# This is the official DeepFaceLab `core/leras/ops/__init__.py` moved
# here verbatim so the torch foundation can own the package namespace.
# It is NOT imported by any torch code path. It is also not importable
# under the Phase 3A torch `nn` (it requires `nn.tf` / `nn.tf_sess`,
# which no longer exist), so it only ever runs in the official TF
# runtime. The torch versions of these ops (dssim, style_loss,
# gaussian blur, pixel_norm, rgb_to_lab, ...) are rebuilt in Phase 3D+;
# the one op already migrated to torch is `depth_to_space`
# (core/leras/ops/__init__.py, Phase 3C).
